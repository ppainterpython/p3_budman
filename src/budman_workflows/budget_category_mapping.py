...
#region Housing, Utilities etc.
    ...
    r'(?i).*BCMUD\s*REC.*': 'Housing-GrapeCove.WaterSewerTrash.Brushy Creek MUD',
...
#endregion Auto
#region Groceries
    r'(?i).*WHOLEFDS.*': 'Groceries.Whole Foods',
    r'(?i).*DOLLAR\s*TREE.*': 'Groceries.Dollar Tree',
    r'(?i).*WAL-MART.*': 'Groceries.Walmart',
    ...
    r'(?i).*TOTAL\s*WINE\s*AND.*': 'Groceries.Liquor.Total Wine and More',
    r'(?i)\bTWIN\sLIQUORS\b': 'Groceries.Liquor.Twin Liquors',
    r'(?i).*KOMEN\sLIQUOR\b': 'Groceries.Liquor.Komen Liquor',
#endregion Groceries
#region Insurance
...
#region Restaurants, Door Dash, Eating Out, Snacks
    r'(?i).*BELLA\s*SERA.*': 'Restaurants.Bella Sera',
    r'(?i).*CASA\s*GARCIA.*': 'Restaurants.Garcias',
    r'(?i).*LANDRYS.*': 'Restaurants.Landrys',
    r'(?i).*FAVOR\s*SONIC.*DRIVEIN.*': 'Restaurants.Favor.Sonic Drive-In',
    r'(?i).*BURGER\s*KING.*': 'Restaurants.Fast Food.Burger King',
    r'(?i).*GATTILAND\s*ROUND\s*ROCK.*': 'Restaurants.Gattiland',
    r'(?i).*COTTON\s*PATCH\s*CAFE.*': 'Restaurants.Cotton Patch Cafe',
    r'(?i).*CHICK\s*FIL\s*A.*': 'Restaurants.Fast Food.Chick Fil A',
    r'(?i).*POPEYES.*': 'Restaurants.Fast Food.Popeyes',
    r'(?i).*P\.\s*TERRY.*': 'Restaurants.Fast Food.P Terrys',
    r'(?i).*RED\s*ROBIN.*': 'Restaurants.Red Robin',
    r'(?i).*FIVE\s*BELOW.*': 'Restaurants.Five Below',
    r'(?i).*PAPPASITO\'S\s*CANTINA.*': 'Restaurants.Pappasitos Cantina',
    r'(?i).*HUNAN\s*RANCH.*': 'Restaurants.Hunan Ranch',
    r'(?i).*WENDY\'*S\s*.*': 'Restaurants.Fast Food.Wendys',
    r'(?i).*DONUT\s*TACO\s*PALACE.*': 'Restaurants.Donut Taco Palace',
    r'(?i).*PANDA\s*EXPRESS.*': 'Restaurants.Fast Food.Panda Express',
    r'(?i).*RAISING\s*CANES.*': 'Restaurants.Fast Food.Raising Canes',
    r'(?i).*FANN\'S\s*PHILLY\s*GRILL.*': 'Restaurants.Fanns Philly Grill',
    r'(?i).*VILLA\s*FRESH\s*ITALI.*': 'Restaurants.Villa Fresh Italian',
    r'(?i).*THUNDERCLOUD.*': 'Restaurants.Fast Food.Thundercloud Subs',
    r'(?i).*FAVOR\s*HOODYS\s*SUBS.*': 'Restaurants.Favor.Hoodys Subs',
    r'(?i).*BLAZE\s*PIZZA.*': 'Restaurants.Fast Food.Blaze Pizza',
    r'(?i).*SANTIAGO\'*S\s*TEX\s*MEX.*': 'Restaurants.Santiagos Tex Mex',
    r'(?i).*CHEESECAKE\s*AUSTIN.*': 'Restaurants.CheeseCake Austin',
    r'(?i).*SALTGRASS\s*ROUND\s*ROCK.*': 'Restaurants.Saltgrass Round Rock',
    r'(?i).*FWSPRINGCAFE.*': 'Restaurants.FW Spring Cafe',
    r'(?i).*BSWH\s*ROUND\s*ROCK\s*CAFE.*': 'Restaurants.BSW Health Round Rock Cafe',
    r'(?i).*LYNNS\s*TABLE.*': 'Restaurants.Lynns Table',
    r'(?i).*PEPPER\s*AND\s*THE\s*PUG.*': 'Restaurants.Pepper and the Pug',
    r'(?i).*Subway.*': 'Restaurants.Fast Food.Subway',
    r'(?i).*HAT\s*CREEK\s*BURGER.*': 'Restaurants.Hat Creek Burgers',
    r'(?i).*POK-*E-*JO.*': 'Restaurants.Poke Jo\'s',
    r'(?i).*HAYLEYCAKESANDCOOKIES.*': 'Restaurants.Haleys Cakes and Cookies',
    r'(?i).*DAHLIA\s*CAFE.*': 'Restaurants.Dahlia Cafe',
    r'(?i).*MESA\s*ROSA\s*MEXICAN.*': 'Restaurants.Mesa Rosa Mexican',
    ...
    r'(?i).*SHIPLEY\s*DO-NUTS': 'Restaurants.Fast Food.Shipleys Donuts',
    r'(?i).*CHIPOTLE.*': 'Restaurants.Fast Food.Chipotle',
    ...
    r'(?i)\bDAIRY\sQUEEN\b': 'Restaurants.Fast Food.Dairy Queen',
    r'(?i)\bCHICK-FIL-A\b': 'Restaurants.Fast Food.Chick-Fil-A',
    r'(?i)\bMOD\sPIZZA\b': 'Restaurants.Mod Pizza',
    r"(?i)\bMCDONALD'S\b": "Restaurants.McDonald's",
    r"(?i)\bMANDOLAS\b": "Restaurants.Fast Food.McDonald's",
    ...
    r'(?i)\bWHATABURGER\s*': "Restaurants.Fast Food.Whataburger",
    r'(?i).*STARBUCKS.*': "Restaurants.Fast Food.Starbucks",
    r'(?i).*JIMMY\s*JOHNS.*': "Restaurants.Fast Food.Jimmy Johns",
    r'(?i).*MIGHTY\s*FINE\s*BURGERS.*': "Restaurants.Fast Food.Might Fine Burgers",
    r'(?i).*HAT\s*CREEK\s*BURGERS.*': "Restaurants.Fast Food.Hat Creek Burgers",
    r'(?i).*LA\s*MARGARITA.*': "Restaurants.La Margarita",
    r'(?i).*PINTHOUSE\s*PIZZA.*': "Restaurants.Pinthouse Pizza",
    r'(?i).*TACO\s*BELL.*': "Restaurants.Fast Food.Taco Bell",
    r'(?i).*MAMA\s*BETTY.*': "Restaurants.Mama Betty's",
