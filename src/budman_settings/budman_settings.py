...
class BudManSettings(metaclass=BDMSingletonMeta):
    ...
    def __repr__(self) -> str:
        return f"<BudManSettings: {self.config.to_dict()}>"
    @property
    ...
    def BDM_STORE_abs_path(self) -> Path:
        ...
        try:
            budman_store_filename_value = self.config[BDM_STORE_FILENAME]
            budman_store_filetype_value = self.config[BDM_STORE_FILETYPE]
            budman_store_full_filename = f"{budman_store_filename_value}{budman_store_filetype_value}"
            budman_folder = self.config[BUDMAN_FOLDER]
