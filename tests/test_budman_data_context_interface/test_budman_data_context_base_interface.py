...
#endregion Globals
# ---------------------------------------------------------------------------- +
def test_budman_data_context_base_interface():
    """Test the BudManDataContextBaseInterface class."""
    try:
        logger.info(test_budman_data_context_base_interface.__doc__)
...
# ---------------------------------------------------------------------------- +
def test_budman_data_context_base_interface_subclass__init__(capsys):
    """Test the BudManDataContextBaseInterface class."""
    class testBudManDC(BudManDataContextBaseInterface):
        ...
        def dc_WORKBOOKS(self, value: bdmns.WORKBOOK_LIST) -> None:
            ...
            return
        @property
        def dc_LOADED_WORKBOOKS(self,capsys) -> bdmns.LOADED_WORKBOOK_LIST:
    ...
    try:
        ...
        assert isinstance(bdmdc, BudManDataContextBaseInterface), \
            "Expected testBudManDC to be a subclass of BudManDataContextBaseInterface, got: " + str(type(bdmdc))
        assert bdmdc.INITIALIZED is True, "Expected INITIALIZED to be True after instantiation"
        assert "Getting INITIALIZED" in capsys.readouterr().out, \
            "Expected 'Getting INITIALIZED' in output, got: " + capsys.readouterr().out
        bdmdc.INITIALIZED = True
        assert "Setting INITIALIZED to True" in capsys.readouterr().out, \
            "Expected 'Setting INITIALIZED to True' in output, got: " + capsys.readouterr().out
        ...
        assert "Setting dc_BUDMAN_STORE to new_budman_store.jsonc" in capsys.readouterr().out, \
            "Expected 'Setting dc_BUDMAN_STORE to new_budman_store.jsonc' in output, got: " + capsys.readouterr().out
        ...
        assert bdmdc.WORKBOOK_loaded("test_wb1") is None, "Expected WORKBOOK_loaded to return None for test_wb1"
        wb = bdmdc.WORKBOOK_load("test_wb1")
        assert isinstance(wb, Workbook), "Expected WORKBOOK_load to return a Workbook instance"
        assert "Loading workbook 'test_wb1'" in capsys.readouterr().out, \
            "Expected 'Loading workbook 'test_wb1'' in output, got: " + capsys.readouterr().out
        bdmdc.WORKBOOK_save("test_wb1", wb)
        assert "Saving workbook 'test_wb1'" in capsys.readouterr().out, \
            "Expected 'Saving workbook 'test_wb1'' in output, got: " + capsys.readouterr().out
        bdmdc.WORKBOOK_remove("test_wb1")
        assert "Removing workbook 'test_wb1'" in capsys.readouterr().out, \
            "Expected 'Removing workbook 'test_wb1'' in output, got: " + capsys.readouterr().out
        bdmdc.WORKBOOK_add("test_wb1", wb)
        assert "Adding workbook 'test_wb1'" in capsys.readouterr().out, \
            "Expected 'Adding workbook 'test_wb1'' in output, got: " + capsys.readouterr().out
        bdmdc.BUDMAN_STORE_load("test_budman_store.jsonc")
        assert "Loading BUDMAN_STORE from test_budman_store.jsonc" in capsys.readouterr().out, \
            "Expected 'Loading BUDMAN_STORE from test_budman_store.jsonc' in output, got: " + capsys.readouterr().out
        bdmdc.BUDMAN_STORE_save("test_budman_store.jsonc")
        assert "Saving BUDMAN_STORE to test_budman_store.jsonc" in capsys.readouterr().out, \
            "Expected 'Saving BUDMAN_STORE to test_budman_store.jsonc' in output, got: " + capsys.readouterr().out
